...
import {
  __getProductRecords,
  ensurePolicyDraftForProduct,
} from './products.js';
import {
  computeLeadTimeBaseline,
  computeServiceLevelBaseline,
  LEAD_TIME_ADJUST_MAX_PCT,
  SERVICE_LEVEL_ADJUST_MAX_PCT,
} from '../services/forecastBaselines.js';
...
const applyLeadTimeCandidate = (baseline: number | null, candidate: number | null): { value: number; note?: string } => {
...
const applyServiceLevelCandidate = (baseline: number, candidate: number | null): { value: number; note?: string } => {
...
export default async function policyRoutes(server: FastifyInstance) {
  ...
  server.post('/recommend-forecast', async (request, reply) => {
    ...
    const normalizedSku =
      typeof body.product?.sku === 'string' ? body.product.sku.trim().toUpperCase() : '';
    ...
    const fallbackLeadTime =
      metricsLeadTime ?? leadTimeBaseline.leadTimeDays ?? DEFAULT_LEAD_TIME_DAYS;
    ...
    if (llmResult) {
      ...
      if (
        ...
        const serviceLevelAdjustment = applyServiceLevelCandidate(
          finalServiceLevelPercent,
          llmServiceLevelRaw,
        );
