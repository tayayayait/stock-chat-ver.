...
import { requeuePendingMovement, takeDuePendingMovements, } from '../stores/pendingMovementsStore.js';
...
export const startPendingMovementScheduler = (intervalMs = DEFAULT_INTERVAL_MS) => {
    if (timer) {
        return;
    }
    processDueMovements();
    timer = setInterval(processDueMovements, intervalMs);
    if (typeof timer.unref === 'function') {
        timer.unref();
    }
};
export const stopPendingMovementScheduler = () => {
    if (timer) {
        clearInterval(timer);
        timer = null;
    }
