...
export const computeLeadTimeBaseline = (input) => {
    ...
    notes.push(`${stats.count}건 리드타임 샘플의 p${percentileUsed}(${rounded}일)을 사용합니다.`);
    return {
...
export const computeServiceLevelBaseline = (input) => {
    const notes = [];
    const normalizedSku = input.sku?.trim().toUpperCase();
    const records = __getProductRecords();
    const product = normalizedSku &&
