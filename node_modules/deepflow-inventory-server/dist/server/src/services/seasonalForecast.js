...
const computeSeasonalFactors = (history, intercept, slope) => {
...
export function buildSeasonalForecast(rawHistory, options = {}) {
    ...
    const fitted = history.map((point, index) => {
    ...
    const absolutePercentageErrors = history
        .map((point, index) => {
        if (point.quantity <= 0) {
            return null;
        }
...
export function estimateStockoutDate(availableStock, futureTimeline) {
