    ...
    try {
        ...
        parsed.forEach((entry) => {
            ...
            policyStore.set(normalizeSku(record.sku), {
                ...
                serviceLevelPercent: typeof record.serviceLevelPercent === 'number' ? record.serviceLevelPercent : null,
...
const sanitizePolicyDraft = (draft) => {
    if (!draft?.sku) {
        return null;
    }
    ...
    const normalizedName = typeof draft.name === 'string'
...
export const listPolicyDrafts = () => Array.from(policyStore.values()).map((record) => ({ ...record }));
export const getPolicyDraft = (sku) => {
    if (!sku) {
        return null;
    }
...
export const savePolicyDrafts = (drafts) => {
...
export const upsertPolicyDraft = (draft) => {
...
export const hasPolicyDraft = (sku) => {
    if (!sku) {
        return false;
    }
...
export const renamePolicyDraft = (currentSku, nextSku, options) => {
...
export const deletePolicyDrafts = (skus) => {
    const removed = [];
    let mutated = false;
...
export const __test__ = {
    ...
    setStoragePathForTests: (filePath) => {
        policyStore.clear();
