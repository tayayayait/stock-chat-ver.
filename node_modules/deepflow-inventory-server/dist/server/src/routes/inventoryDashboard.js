...
import { getDailyMovementHistory, summarizeMovementTotals, } from '../stores/movementAnalyticsStore.js';
import { __getProductRecords } from './products.js';
const RISK_STABLE = '\uC815\uC0C1';
const RISK_SHORTAGE = '\uACB0\uD488\uC704\uD5D8';
const RISK_OVERSTOCK = '\uACFC\uC694';
const RISK_ORDER = [RISK_STABLE, RISK_SHORTAGE, RISK_OVERSTOCK];
const normalizeSku = (value) => value.trim().toUpperCase();
const SERVICE_LEVEL_Z_TABLE = [
    { percent: 90, z: 1.2816 },
    { percent: 95, z: 1.6449 },
    { percent: 98, z: 2.0537 },
    { percent: 99, z: 2.3263 },
...
const resolveServiceLevelPercent = (product) => {
    switch (product.risk) {
        case RISK_SHORTAGE:
            return 98;
        case RISK_OVERSTOCK:
...
const resolveLeadTimeDays = (product) => {
...
const computeSafetyStock = (product) => {
    const policy = getPolicyDraft(product.sku);
    const sigmaCandidate = policy?.demandStdDev;
    const sigma = Number.isFinite(sigmaCandidate ?? NaN) && (sigmaCandidate ?? 0) > 0
        ? Math.max(sigmaCandidate, 0)
    ...
    const serviceLevelPercent = Number.isFinite(serviceLevelCandidate ?? NaN) && (serviceLevelCandidate ?? 0) > 0
        ? serviceLevelCandidate
    ...
    const leadTimeDays = Number.isFinite(leadTimeCandidate ?? NaN) && (leadTimeCandidate ?? 0) > 0
        ? leadTimeCandidate
    ...
    const rho = Number.isFinite(correlationCandidate ?? NaN) && (correlationCandidate ?? 0) >= 0
...
const toSafeNumber = (value) => Number.isFinite(value) ? Math.max(0, Number(value)) : 0;
const calculateAvailable = (onHand, reserved) => Math.max(onHand - reserved, 0);
const clamp = (value, min = 0, max = Number.POSITIVE_INFINITY) => Math.min(max, Math.max(min, value));
const selectPrimaryLocation = (inventory) => {
...
const buildTrendSeries = (history, currentAvailable) => {
...
const aggregateMovementHistory = (histories) => {
...
const toUtcDateOnly = (value) => new Date(Date.UTC(value.getUTCFullYear(), value.getUTCMonth(), value.getUTCDate()));
...
const parseAnalysisRange = (params) => {
...
const zeroFillDailySeries = (range, points) => {
...
const getWeekEnd = (weekStart) => addUtcDays(weekStart, 6);
const getMonthStart = (value) => new Date(Date.UTC(value.getUTCFullYear(), value.getUTCMonth(), 1));
const getMonthEnd = (monthStart) => new Date(Date.UTC(monthStart.getUTCFullYear(), monthStart.getUTCMonth() + 1, 0));
const formatPeriodLabel = (start, end, groupBy) => {
    if (groupBy === 'month') {
...
const buildStockTrajectory = (daily, startingOnHand, currentReserved, safetyStock) => {
...
const groupDailySeries = (daily, groupBy, startingOnHand, currentReserved, safetyStock) => {
...
const summarizeProductForWarehouse = (product, warehouseCode) => {
...
const aggregateTotalsForWarehouse = (products, warehouseCode) => {
    const accumulator = products.reduce((totals, product) => {
...
const computeProjectedStockoutDate = (available, avgDailyOutbound) => {
...
export default async function inventoryDashboardRoutes(server) {
    server.get('/dashboard', async (_request, reply) => {
        const products = __getProductRecords();
        const skuCount = products.length;
        const movementHistoryBySku = new Map();
        products.forEach((product) => {
            movementHistoryBySku.set(product.sku, getDailyMovementHistory({ days: DEFAULT_MOVEMENT_WINDOW_DAYS, sku: product.sku }));
        });
        ...
        const avgDaysOfSupply = dosSamples.length > 0 ? Math.round(dosSamples.reduce((sum, value) => sum + value, 0) / dosSamples.length) : 0;
        ...
        const warehouseAccumulator = new Map();
        products.forEach((product) => {
        ...
        const safetyStockFor = (product) => computeSafetyStock(product);
    ...
    server.get('/analysis', async (request, reply) => {
        ...
        const scope = typeof query.warehouseCode === 'string' && query.warehouseCode.trim() ? query.warehouseCode.trim() : null;
        const groupBy = resolveGroupBy(range, typeof query.groupBy === 'string' ? query.groupBy : undefined);
        const skuCandidate = typeof query.sku === 'string' && query.sku.trim().length > 0 ? normalizeSku(query.sku) : null;
        const products = __getProductRecords();
        const scopedProducts = skuCandidate !== null
        ...
        const periodSeries = groupDailySeries(dailySeries, groupBy, startingOnHand, currentTotals.reserved, currentTotals.safetyStock);
    ...
    server.get('/warehouse-items', async (request, reply) => {
        const query = (request.query ?? {});
        const warehouseCode = typeof query.warehouseCode === 'string' && query.warehouseCode.trim() ? query.warehouseCode.trim() : null;
