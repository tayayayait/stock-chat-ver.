...
function validateLocationPayload(input) {
    ...
    const errors = [];
    ['code', 'warehouseCode', 'description'].forEach((field) => {
...
export default async function locationsRoutes(server) {
    ensureLocationSeedData();
    server.get('/', async (request, reply) => {
        const { warehouseCode } = (request.query ?? {});
        const items = listLocations().filter((item) => warehouseCode ? item.warehouseCode === warehouseCode : true);
