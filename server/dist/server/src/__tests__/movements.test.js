...
async function main() {
    ...
    try {
        ...
        const adjustBody = adjustResponse.json();
        ...
        const futureBody = futureResponse.json();
        ...
        const transferListBody = transferList.json();
        ...
        const warehouseBody = warehouseList.json();
        assert.ok(warehouseBody.items.length >= 2);
        assert.ok(warehouseBody.balances.length >= 2);
        const whB = warehouseBody.balances.find((item) => item.warehouse === 'WH-BUSAN');
        assert.ok(whB);
        ...
        const productMovementBody = productMovement.json();
