...
const sanitizeItems = (items) => items
    ...
    const kpiTarget = typeof item.kpi?.target === 'number' || typeof item.kpi?.target === 'string'
        ? item.kpi.target
        : '';
    ...
    .filter((item) => Boolean(item && item.kpi));
export function listActionPlans(filter = {}) {
...
export function updateActionPlan(id, patch) {
...
export function transitionActionPlanStatus(id, nextStatus, actor) {
    const existing = actionPlanStore.get(id);
    if (!existing) {
        throw new Error(`Action plan ${id} not found`);
    }
