import { createPartner, deletePartner, listPartners, updatePartner, } from '../stores/partnersStore.js';
...
const buildErrorResponse = (reply, status, message) => reply.status(status).send({ success: false, error: message });
export default async function partnersRoutes(server) {
    server.get('/', (request, reply) => {
        const { type, includeSample } = request.query;
